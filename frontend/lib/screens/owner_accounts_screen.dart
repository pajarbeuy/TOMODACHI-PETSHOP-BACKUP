...
class _OwnerAccountsScreenState extends State<OwnerAccountsScreen> {
  ...
  final _roles = const {'Owner': 1, 'Kasir': 2, 'Admin': 3};
  ...
  void _openForm({AccountModel? account}) {
    ...
    final initialRole = account != null ? _capitalize(account.roleName) : 'Kasir';
    // If we are creating, or editing a non-owner, we don't allow selecting Owner.
    if (_creating || initialRole != 'Owner') {
      _selectedRole = initialRole == 'Owner' ? 'Kasir' : initialRole;
    } else {
      _selectedRole = initialRole; // Editing the existing owner
    }
  ...
  Future<void> _save() async {
    ...
    if (_creating && _passwordCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      if (_creating) {
        ...
        final ok = await widget.authService.updateAccount(
          ...
          passwordConfirmation: _passwordCtrl.text.isEmpty
              ? null
          ...
          .map(
            (a) => DataRow(
              cells: [
                ...
                DataCell(Text(_capitalize(a.roleName))),
                DataCell(
                  Row(
                    children: [
                      IconButton(
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(account.name, style: _text(weight: FontWeight.w700)),
            Text(account.email),
            Text(_capitalize(account.roleName)),
            Row(
  ...
  Widget _buildForm(StateSetter setModalState) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(
          _creating ? 'Tambah Akun' : 'Ubah Akun',
        ...
        DropdownButtonFormField<String>(
          value: _selectedRole,
          items: _roles.keys
              .where((r) => r != 'Owner' || _selectedRole == 'Owner')
              .map((r) => DropdownMenuItem(value: r, child: Text(r)))
              .toList(),
          onChanged: (v) {
            setModalState(() => _selectedRole = v ?? 'Kasir');
            setState(() => _selectedRole = v ?? 'Kasir');
          },
