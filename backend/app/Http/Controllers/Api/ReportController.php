...
class ReportController extends Controller
{
    ...
    public function analytics(Request $request)
    {
        ...
        $data = Cache::remember($cacheKey, $ttl, function () use ($trendDays) {
            ...
            // 2. Sales Trend — query 1x dengan GROUP BY, bukan loop N query
            $trendRows = Transaction::where('status', 'completed')
                ->whereDate('created_at', '>=', now()->subDays($trendDays - 1)->toDateString())
                ->select(
                    DB::raw('DATE(created_at) as date'),
            ...
            for ($i = $trendDays - 1; $i >= 0; $i--) {
                ...
                $trendData[] = [
                    ...
                    'transactions' => intval($row?->transactions ?? 0),
                ];
            }
